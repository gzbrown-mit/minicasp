...
import minicasp.chem
from minicasp.utils.loading import load_dynamic_class

if TYPE_CHECKING:
    from minicasp.chem import RetroReaction
    from minicasp.utils.type_utils import Any, Dict, Optional, Sequence, StrDict
...
class MoleculeSerializer:
    ...
    def __getitem__(self, mol: Optional[minicasp.chem.Molecule]) -> Optional[int]:
        if mol is None:
            return None
    ...
    def _add_mol(self, mol: minicasp.chem.Molecule) -> None:
        id_ = id(mol)
        dict_ = {"smiles": mol.smiles, "class": mol.__class__.__name__}
        if isinstance(mol, minicasp.chem.TreeMolecule):
...
class MoleculeDeserializer:
    ...
    def __getitem__(self, id_: Optional[int]) -> Optional[minicasp.chem.Molecule]:
        if id_ is None:
            return None
    ...
    def get_tree_molecules(
    ...
    ) -> Sequence[minicasp.chem.TreeMolecule]:
        """
        ...
        for id_ in ids:
            obj = self[id_]
            if obj is None or not isinstance(obj, minicasp.chem.TreeMolecule):
    ...
    def _create_molecules(self, store: dict) -> None:
        for id_, spec in store.items():
            ...
            kwargs = dict(spec)
            del kwargs["class"]
            self._objects[id_] = getattr(minicasp.chem, cls)(**kwargs)
...
def deserialize_action(
    ...
    try:
        ...
        class_spec = "minicasp.chem.TemplatedRetroReaction"
